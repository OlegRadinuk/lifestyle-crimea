// data/apartments.ts

export type Apartment = {
  id: string;
  title: string;
  shortDescription: string;
  description: string;
  maxGuests: number;
  area: number;
  priceBase: number;
  features: string[];
  images: string[];
  view: 'sea' | 'mountain' | 'city' | 'garden' | 'forest' | 'mixed';
  hasTerrace: boolean;
  floor?: string;
  beds: string;
  petFriendly?: boolean;
  hasWorkplace?: boolean;
  hasSafe?: boolean;
  hasHeatedFloor?: boolean;
  hasSlippers?: boolean;
  hasBathrobes?: boolean;
  hasWashingMachine?: boolean;
  hasCoffeeMachine?: boolean;
  hasMicrowave?: boolean;
  hasStove?: boolean;
  hasDishwasher?: boolean;
  hasBalcony?: boolean;
};

export const APARTMENTS: Apartment[] = [
  // Дизайнерские студии (30 шт)
  {
    id: 'ls-space',
    title: 'LS-SPACE',
    shortDescription: 'Дизайнерская студия с видом на море',
    description: 'Просторная студия с современным дизайном и панорамным видом на море.',
    maxGuests: 3,
    area: 36,
    priceBase: 6500,
    view: 'sea',
    hasTerrace: true,
    floor: '',
    beds: '1 кресло-кровать + 1 двуспальная',
    features: ['Интернет', 'Холодильник', 'Кондиционер', 'Фен', 'Балкон', 'Кухня', 'Микроволновка', 'Плита'],
    images: ['/images/apartments/space/1.webp', '/images/apartments/space/2.webp', '/images/apartments/space/3.webp'],
    hasMicrowave: true,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: false,
    hasWashingMachine: false,
    hasHeatedFloor: false,
    hasSlippers: false,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: false
  },
  {
    id: 'ls-coffee-ice-cream',
    title: 'LS-COFFEE ICE CREAM',
    shortDescription: 'Студия со вкусом мороженого с видом на горы',
    description: 'Уютная студия с оригинальным дизайном и прекрасным видом на горы.',
    maxGuests: 3,
    area: 33,
    priceBase: 6000,
    view: 'mountain',
    hasTerrace: true,
    floor: '',
    beds: '1 односпальная + 1 двуспальная',
    features: ['Интернет', 'Холодильник', 'Кондиционер', 'Душевая кабина', 'Балкон', 'Кухня'],
    images: ['/images/apartments/coffee-ice-cream/1.webp', '/images/apartments/coffee-ice-cream/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: false,
    hasWashingMachine: false,
    hasHeatedFloor: false,
    hasSlippers: false,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: true
  },
  {
    id: 'ls-summer-emotions',
    title: 'LS-SUMMER EMOTIONS',
    shortDescription: 'Нежные эмоции лета с видом на горы',
    description: 'Студия с тёплой атмосферой и панорамным видом на горы с 4 этажа.',
    maxGuests: 3,
    area: 30,
    priceBase: 5800,
    view: 'mountain',
    hasTerrace: true,
    floor: '4',
    beds: '1 кресло-кровать + 1 двуспальная',
    features: ['Интернет', 'Микроволновка', 'Кондиционер', 'Душевая кабина', 'Кофемашина', 'Пол с подогревом', 'Тапочки'],
    images: ['/images/apartments/summer-emotions/1.webp', '/images/apartments/summer-emotions/2.webp'],
    hasMicrowave: true,
    hasStove: false,
    hasBalcony: true,
    hasCoffeeMachine: true,
    hasWashingMachine: false,
    hasHeatedFloor: true,
    hasSlippers: true,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: false
  },
  {
    id: 'ls-black-strong',
    title: 'LS-BLACK STRONG',
    shortDescription: 'Экстремальное лето с видом на море и горы',
    description: 'Стильная студия на 7 этаже с видом на море и горы.',
    maxGuests: 3,
    area: 34,
    priceBase: 6200,
    view: 'mixed',
    hasTerrace: true,
    floor: '7',
    beds: '1 диван-кровать + 1 двуспальная',
    features: ['Интернет', 'Микроволновка', 'Кондиционер', 'Душевая кабина', 'Кофемашина', 'Пол с подогревом', 'Тапочки'],
    images: ['/images/apartments/black-strong/1.webp', '/images/apartments/black-strong/2.webp'],
    hasMicrowave: true,
    hasStove: false,
    hasBalcony: true,
    hasCoffeeMachine: true,
    hasWashingMachine: false,
    hasHeatedFloor: true,
    hasSlippers: true,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: false
  },
  {
    id: 'ls-deep-music',
    title: 'LS-DEEP MUSIC',
    shortDescription: 'Звуки лета с видом на море',
    description: 'Студия на 13 этаже с панорамным видом на море и современным интерьером.',
    maxGuests: 3,
    area: 28,
    priceBase: 5500,
    view: 'sea',
    hasTerrace: true,
    floor: '13',
    beds: '1 кресло + 1 двуспальная',
    features: ['Холодильник', 'Кондиционер', 'Банные халаты', 'Душ', 'Кофемашина', 'Рабочее место', 'Пол с подогревом', 'Полотенца для животных'],
    images: ['/images/apartments/deep-music/1.webp', '/images/apartments/deep-music/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: true,
    hasWashingMachine: true,
    hasHeatedFloor: true,
    hasSlippers: true,
    hasBathrobes: true,
    hasSafe: true,
    hasWorkplace: true,
    petFriendly: true
  },
  {
    id: 'ls-dream-vacation',
    title: 'LS-DREAM VACATION',
    shortDescription: 'Отдых мечты с видом на город и горы',
    description: 'Студия на 5 этаже с прекрасным видом на город и горы.',
    maxGuests: 3,
    area: 32,
    priceBase: 5900,
    view: 'mixed',
    hasTerrace: true,
    floor: '5',
    beds: '1 кресло + 1 двуспальная',
    features: ['Кондиционер', 'Микроволновка', 'Душевая кабина', 'Кофемашина', 'Балкон', 'Пол с подогревом', 'Тапочки'],
    images: ['/images/apartments/dream-vacation/1.webp', '/images/apartments/dream-vacation/2.webp'],
    hasMicrowave: true,
    hasStove: false,
    hasBalcony: true,
    hasCoffeeMachine: true,
    hasWashingMachine: false,
    hasHeatedFloor: true,
    hasSlippers: true,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: false
  },
  {
    id: 'ls-econom-studio',
    title: 'LS-ECONOM STUDIO',
    shortDescription: 'Экономный отдых с видом на горы',
    description: 'Компактная студия на 7 этаже с видом на горы.',
    maxGuests: 3,
    area: 28,
    priceBase: 5200,
    view: 'mountain',
    hasTerrace: true,
    floor: '7',
    beds: '1 кресло + 1 двуспальная',
    features: ['Интернет', 'Холодильник', 'Кондиционер', 'Душ', 'Кофемашина', 'Сейф', 'Банные халаты', 'Пол с подогревом'],
    images: ['/images/apartments/econom-studio/1.webp', '/images/apartments/econom-studio/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: true,
    hasWashingMachine: false,
    hasHeatedFloor: true,
    hasSlippers: true,
    hasBathrobes: true,
    hasSafe: true,
    hasWorkplace: false,
    petFriendly: false
  },
  {
    id: 'ls-family-comfort',
    title: 'LS-FAMILY COMFORT',
    shortDescription: 'Летние теплые дни с видом на горы',
    description: 'Уютная студия на 4 этаже с видом на горы.',
    maxGuests: 3,
    area: 32,
    priceBase: 5800,
    view: 'mountain',
    hasTerrace: true,
    floor: '4',
    beds: '1 кресло + 1 двуспальная',
    features: ['Кондиционер', 'Микроволновка', 'Душевая кабина', 'Кофемашина', 'Балкон', 'Пол с подогревом', 'Тапочки'],
    images: ['/images/apartments/family-comfort/1.webp', '/images/apartments/family-comfort/2.webp'],
    hasMicrowave: true,
    hasStove: false,
    hasBalcony: true,
    hasCoffeeMachine: true,
    hasWashingMachine: false,
    hasHeatedFloor: true,
    hasSlippers: true,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: false
  },
  {
    id: 'ls-in-the-moment',
    title: 'LS-IN THE MOMENT',
    shortDescription: 'Жизнь в моменте с видом на море',
    description: 'Студия на 11 этаже с панорамным видом на море.',
    maxGuests: 3,
    area: 32,
    priceBase: 6000,
    view: 'sea',
    hasTerrace: true,
    floor: '11',
    beds: '1 кресло + 1 двуспальная',
    features: ['Микроволновка', 'Кондиционер', 'Душевая кабина', 'Кофемашина', 'Стиральная машина', 'Сейф', 'Пол с подогревом', 'Тапочки'],
    images: ['/images/apartments/in-the-moment/1.webp', '/images/apartments/in-the-moment/2.webp'],
    hasMicrowave: true,
    hasStove: false,
    hasBalcony: true,
    hasCoffeeMachine: true,
    hasWashingMachine: true,
    hasHeatedFloor: true,
    hasSlippers: true,
    hasBathrobes: false,
    hasSafe: true,
    hasWorkplace: false,
    petFriendly: true
  },
  {
    id: 'ls-lux-flower-kiss',
    title: 'LS-LUX-FLOWER KISS',
    shortDescription: 'В ярких оттенках лета с видом на море',
    description: 'Студия на 2 этаже с видом на море и ярким дизайном.',
    maxGuests: 3,
    area: 34,
    priceBase: 6300,
    view: 'sea',
    hasTerrace: true,
    floor: '2',
    beds: '1 кресло + 1 двуспальная',
    features: ['Холодильник', 'Кондиционер', 'Душ', 'Банные халаты', 'Кофемашина', 'Сейф', 'Пол с подогревом', 'Тапочки'],
    images: ['/images/apartments/lux-flower-kiss/1.webp', '/images/apartments/lux-flower-kiss/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: true,
    hasWashingMachine: true,
    hasHeatedFloor: true,
    hasSlippers: true,
    hasBathrobes: true,
    hasSafe: true,
    hasWorkplace: true,
    petFriendly: false
  },
  {
    id: 'ls-relax-time',
    title: 'LS-RELAX TIME',
    shortDescription: 'Время для отдыха с видом на море и горы',
    description: 'Студия на 14 этаже с видом на море и горы.',
    maxGuests: 3,
    area: 33,
    priceBase: 6100,
    view: 'mixed',
    hasTerrace: true,
    floor: '14',
    beds: '1 кресло + 1 двуспальная',
    features: ['Интернет', 'Холодильник', 'Кондиционер', 'Душевая кабина', 'Кофемашина', 'Балкон'],
    images: ['/images/apartments/relax-time/1.webp', '/images/apartments/relax-time/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: true,
    hasWashingMachine: false,
    hasHeatedFloor: false,
    hasSlippers: false,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: true
  },
  {
    id: 'ls-sweet-summer',
    title: 'LS-SWEET SUMMER',
    shortDescription: 'Сладкие эмоции лета с видом на море',
    description: 'Студия на 12 этаже с видом на море.',
    maxGuests: 3,
    area: 32,
    priceBase: 5900,
    view: 'sea',
    hasTerrace: true,
    floor: '12',
    beds: '1 кресло + 1 двуспальная',
    features: ['Микроволновка', 'Кондиционер', 'Душевая кабина', 'Кофемашина', 'Балкон', 'Пол с подогревом', 'Тапочки'],
    images: ['/images/apartments/sweet-summer/1.webp', '/images/apartments/sweet-summer/2.webp'],
    hasMicrowave: true,
    hasStove: false,
    hasBalcony: true,
    hasCoffeeMachine: true,
    hasWashingMachine: false,
    hasHeatedFloor: true,
    hasSlippers: true,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: false
  },
  {
    id: 'ls-comfort-home',
    title: 'LS-COMFORT HOME',
    shortDescription: 'С комфортом с видом на море',
    description: 'Просторная студия с видом на море.',
    maxGuests: 2,
    area: 36,
    priceBase: 6200,
    view: 'sea',
    hasTerrace: true,
    floor: '',
    beds: '1 двуспальная',
    features: ['Холодильник', 'Кондиционер', 'Фен', 'Балкон', 'Кухня', 'Плита', 'Стиральная машина', 'Кофемашина'],
    images: ['/images/apartments/comfort-home/1.webp', '/images/apartments/comfort-home/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: true,
    hasWashingMachine: true,
    hasHeatedFloor: false,
    hasSlippers: false,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: false
  },
  {
    id: 'ls-lux-sweet-caramel',
    title: 'LS-LUX-SWEET CARAMEL',
    shortDescription: 'Сладкий вкус лета с видом на море',
    description: 'Студия на 3 этаже с видом на море.',
    maxGuests: 3,
    area: 28,
    priceBase: 5500,
    view: 'sea',
    hasTerrace: true,
    floor: '3',
    beds: '1 кресло + 1 двуспальная',
    features: ['Холодильник', 'Кондиционер', 'Фен', 'Балкон', 'Кухня', 'Стиральная машина', 'Кофемашина'],
    images: ['/images/apartments/lux-sweet-caramel/1.webp', '/images/apartments/lux-sweet-caramel/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: true,
    hasWashingMachine: true,
    hasHeatedFloor: false,
    hasSlippers: false,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: false
  },
  {
    id: 'ls-steel-love',
    title: 'LS-STEEL LOVE',
    shortDescription: 'В стальных оттенках лета с видом на море',
    description: 'Студия на 13 этаже с видом на море.',
    maxGuests: 3,
    area: 32,
    priceBase: 6000,
    view: 'sea',
    hasTerrace: true,
    floor: '13',
    beds: '1 кресло + 1 двуспальная',
    features: ['Кондиционер', 'Душ', 'Кофемашина', 'Сейф', 'Рабочее место', 'Пол с подогревом'],
    images: ['/images/apartments/steel-love/1.webp', '/images/apartments/steel-love/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: true,
    hasWashingMachine: true,
    hasHeatedFloor: true,
    hasSlippers: true,
    hasBathrobes: true,
    hasSafe: true,
    hasWorkplace: true,
    petFriendly: false
  },
  {
    id: 'ls-art-crystal-blue',
    title: 'LS-ART-CRYSTAL BLUE',
    shortDescription: 'Кристально синее небо с видом на море',
    description: 'Студия на 13 этаже с видом на море.',
    maxGuests: 3,
    area: 32,
    priceBase: 5900,
    view: 'sea',
    hasTerrace: true,
    floor: '13',
    beds: '1 кресло + 1 двуспальная',
    features: ['Холодильник', 'Кондиционер', 'Душевая кабина', 'Балкон', 'Кухня'],
    images: ['/images/apartments/art-crystal-blue/1.webp', '/images/apartments/art-crystal-blue/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: false,
    hasWashingMachine: false,
    hasHeatedFloor: false,
    hasSlippers: false,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: true
  },
  {
    id: 'ls-art-olive',
    title: 'LS-ART-OLIVE',
    shortDescription: 'В оливковых оттенках лета с видом на море',
    description: 'Студия на 13 этаже с видом на море.',
    maxGuests: 3,
    area: 32,
    priceBase: 5900,
    view: 'sea',
    hasTerrace: true,
    floor: '13',
    beds: '1 кресло + 1 двуспальная',
    features: ['Холодильник', 'Кондиционер', 'Душ', 'Кофемашина', 'Сейф', 'Пол с подогревом', 'Тапочки'],
    images: ['/images/apartments/art-olive/1.webp', '/images/apartments/art-olive/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: true,
    hasWashingMachine: false,
    hasHeatedFloor: true,
    hasSlippers: true,
    hasBathrobes: true,
    hasSafe: true,
    hasWorkplace: false,
    petFriendly: false
  },
  {
    id: 'ls-blue-curacao',
    title: 'LS-BLUE CURACAO',
    shortDescription: 'Горько-сладкий вкус лета с видом на море',
    description: 'Студия на 3 этаже с видом на море.',
    maxGuests: 3,
    area: 29,
    priceBase: 5500,
    view: 'sea',
    hasTerrace: true,
    floor: '3',
    beds: '1 диван-кровать + 1 двуспальная',
    features: ['Холодильник', 'Кондиционер', 'Душевая кабина', 'Кофемашина'],
    images: ['/images/apartments/blue-curacao/1.webp', '/images/apartments/blue-curacao/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: true,
    hasWashingMachine: false,
    hasHeatedFloor: false,
    hasSlippers: false,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: false
  },
  {
    id: 'ls-blueberry',
    title: 'LS-BLUEBERRY',
    shortDescription: 'Ягодных оттенков лета с видом на море',
    description: 'Студия на 4 этаже с видом на море.',
    maxGuests: 3,
    area: 34,
    priceBase: 6100,
    view: 'sea',
    hasTerrace: true,
    floor: '4',
    beds: '1 кресло + 1 двуспальная',
    features: ['Холодильник', 'Кондиционер', 'Душевая кабина', 'Кофемашина'],
    images: ['/images/apartments/blueberry/1.webp', '/images/apartments/blueberry/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: true,
    hasWashingMachine: false,
    hasHeatedFloor: false,
    hasSlippers: false,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: false
  },
  {
    id: 'ls-cool-lemonade',
    title: 'LS-COOL LEMONADE',
    shortDescription: 'Освежающий вкус лимонада с видом на море',
    description: 'Студия на 6 этаже с видом на море.',
    maxGuests: 3,
    area: 29,
    priceBase: 5600,
    view: 'sea',
    hasTerrace: true,
    floor: '6',
    beds: '1 кресло + 1 двуспальная',
    features: ['Холодильник', 'Кондиционер', 'Душевая кабина', 'Кофемашина', 'Полотенца для животных'],
    images: ['/images/apartments/cool-lemonade/1.webp', '/images/apartments/cool-lemonade/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: true,
    hasWashingMachine: false,
    hasHeatedFloor: false,
    hasSlippers: false,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: true
  },
  {
    id: 'ls-green',
    title: 'LS-GREEN',
    shortDescription: 'В ярких оттенках лета с боковым видом на море',
    description: 'Студия на 13 этаже с боковым видом на море.',
    maxGuests: 3,
    area: 33,
    priceBase: 5800,
    view: 'sea',
    hasTerrace: true,
    floor: '13',
    beds: '1 кресло + 1 двуспальная',
    features: ['Кондиционер', 'Микроволновка', 'Душ', 'Сейф', 'Полотенца для животных'],
    images: ['/images/apartments/green/1.webp', '/images/apartments/green/2.webp'],
    hasMicrowave: true,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: false,
    hasWashingMachine: true,
    hasHeatedFloor: false,
    hasSlippers: true,
    hasBathrobes: true,
    hasSafe: true,
    hasWorkplace: false,
    petFriendly: true
  },
  {
    id: 'ls-hi-tech-emotion',
    title: 'LS-HI-TECH EMOTION',
    shortDescription: 'Светлые эмоции лета с видом на море',
    description: 'Студия на 2 этаже с видом на море.',
    maxGuests: 3,
    area: 32,
    priceBase: 5900,
    view: 'sea',
    hasTerrace: true,
    floor: '2',
    beds: '1 кресло + 1 двуспальная',
    features: ['Холодильник', 'Кондиционер', 'Душевая кабина', 'Микроволновка', 'Фен'],
    images: ['/images/apartments/hi-tech-emotion/1.webp', '/images/apartments/hi-tech-emotion/2.webp'],
    hasMicrowave: true,
    hasStove: false,
    hasBalcony: true,
    hasCoffeeMachine: false,
    hasWashingMachine: false,
    hasHeatedFloor: false,
    hasSlippers: false,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: false
  },
  {
    id: 'ls-hi-tech-relax',
    title: 'LS-HI-TECH RELAX',
    shortDescription: 'Свобода лета с видом на море',
    description: 'Студия на 2 этаже с видом на море.',
    maxGuests: 3,
    area: 27,
    priceBase: 5300,
    view: 'sea',
    hasTerrace: true,
    floor: '2',
    beds: '1 кресло + 1 двуспальная',
    features: ['Холодильник', 'Кондиционер', 'Душевая кабина', 'Микроволновка', 'Фен'],
    images: ['/images/apartments/hi-tech-relax/1.webp', '/images/apartments/hi-tech-relax/2.webp'],
    hasMicrowave: true,
    hasStove: false,
    hasBalcony: true,
    hasCoffeeMachine: false,
    hasWashingMachine: false,
    hasHeatedFloor: false,
    hasSlippers: false,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: false
  },
  {
    id: 'ls-lux-only-you',
    title: 'LS-LUX-ONLY YOU',
    shortDescription: 'Только ты с видом на море',
    description: 'Студия на 4 этаже с видом на море.',
    maxGuests: 3,
    area: 32,
    priceBase: 6000,
    view: 'sea',
    hasTerrace: true,
    floor: '4',
    beds: '1 кресло + 1 двуспальная',
    features: ['Кондиционер', 'Душевая кабина', 'Сейф', 'Кухня', 'Плита', 'Полотенца для животных'],
    images: ['/images/apartments/lux-only-you/1.webp', '/images/apartments/lux-only-you/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: false,
    hasWashingMachine: true,
    hasHeatedFloor: false,
    hasSlippers: false,
    hasBathrobes: false,
    hasSafe: true,
    hasWorkplace: false,
    petFriendly: true
  },
  {
    id: 'ls-lux-fly-sky',
    title: 'LS-LUX-FLY SKY',
    shortDescription: 'Прямой вид на море с 16 этажа',
    description: 'Студия на 16 этаже с прямым видом на море.',
    maxGuests: 2,
    area: 32,
    priceBase: 6500,
    view: 'sea',
    hasTerrace: false,
    floor: '16',
    beds: '1 двуспальная',
    features: [],
    images: ['/images/apartments/lux-fly-sky/1.webp', '/images/apartments/lux-fly-sky/2.webp'],
    hasMicrowave: false,
    hasStove: false,
    hasBalcony: false,
    hasCoffeeMachine: false,
    hasWashingMachine: false,
    hasHeatedFloor: false,
    hasSlippers: false,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: false
  },
  {
    id: 'ls-lux-beautiful-days',
    title: 'LS-LUX-BEAUTIFUL DAYS',
    shortDescription: 'Прекрасные дни с видом на горы и море',
    description: 'Студия на 15 этаже с видом на горы и море.',
    maxGuests: 3,
    area: 36,
    priceBase: 6800,
    view: 'mixed',
    hasTerrace: true,
    floor: '15',
    beds: '1 кресло + 1 двуспальная',
    features: ['Холодильник', 'Кондиционер', 'Фен', 'Кухня', 'Стиральная машина', 'Кофемашина'],
    images: ['/images/apartments/lux-beautiful-days/1.webp', '/images/apartments/lux-beautiful-days/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: true,
    hasWashingMachine: true,
    hasHeatedFloor: false,
    hasSlippers: false,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: false
  },
  {
    id: 'ls-lux-fly-mood',
    title: 'LS-LUX-FLY MOOD',
    shortDescription: 'Прямой вид на море с 16 этажа',
    description: 'Студия на 16 этаже с прямым видом на море.',
    maxGuests: 2,
    area: 36,
    priceBase: 6800,
    view: 'sea',
    hasTerrace: false,
    floor: '16',
    beds: '1 двуспальная',
    features: [],
    images: ['/images/apartments/lux-fly-mood/1.webp', '/images/apartments/lux-fly-mood/2.webp'],
    hasMicrowave: false,
    hasStove: false,
    hasBalcony: false,
    hasCoffeeMachine: false,
    hasWashingMachine: false,
    hasHeatedFloor: false,
    hasSlippers: false,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: false
  },
  {
    id: 'ls-lux-sun-rays',
    title: 'LS-LUX-SUN RAYS',
    shortDescription: 'Солнечные лучи с видом на море',
    description: 'Студия на 15 этаже с видом на море.',
    maxGuests: 3,
    area: 36,
    priceBase: 6700,
    view: 'sea',
    hasTerrace: true,
    floor: '15',
    beds: '1 кресло + 1 двуспальная',
    features: ['Холодильник', 'Кондиционер', 'Фен', 'Кухня', 'Стиральная машина', 'Кофемашина'],
    images: ['/images/apartments/lux-sun-rays/1.webp', '/images/apartments/lux-sun-rays/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: true,
    hasWashingMachine: true,
    hasHeatedFloor: false,
    hasSlippers: false,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: false
  },
  {
    id: 'ls-lux-sunny-mood',
    title: 'LS-LUX-SUNNY MOOD',
    shortDescription: 'Отличное настроение с видом на горы и море',
    description: 'Студия на 15 этаже с видом на горы и море.',
    maxGuests: 3,
    area: 36,
    priceBase: 6800,
    view: 'mixed',
    hasTerrace: true,
    floor: '15',
    beds: '1 кресло + 1 двуспальная',
    features: ['Холодильник', 'Кондиционер', 'Фен', 'Кухня', 'Стиральная машина', 'Кофемашина'],
    images: ['/images/apartments/lux-sunny-mood/1.webp', '/images/apartments/lux-sunny-mood/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: true,
    hasWashingMachine: true,
    hasHeatedFloor: false,
    hasSlippers: false,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: false
  },
  {
    id: 'ls-lux-fly-blue-light',
    title: 'LS-LUX-FLY BLUE LIGHT',
    shortDescription: 'Прямой вид на море с 16 этажа',
    description: 'Студия на 16 этаже с прямым видом на море.',
    maxGuests: 3,
    area: 32,
    priceBase: 6400,
    view: 'sea',
    hasTerrace: false,
    floor: '16',
    beds: '1 диван-кровать + 1 двуспальная',
    features: [],
    images: ['/images/apartments/lux-fly-blue-light/1.webp', '/images/apartments/lux-fly-blue-light/2.webp'],
    hasMicrowave: false,
    hasStove: false,
    hasBalcony: false,
    hasCoffeeMachine: false,
    hasWashingMachine: false,
    hasHeatedFloor: false,
    hasSlippers: false,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: false
  },

  // Апартаменты с отдельной спальней (9 шт)
  {
    id: 'ls-diamond-green',
    title: 'LS-DIAMOND GREEN',
    shortDescription: 'Алмазного свечения лета с видом на море и горы',
    description: 'Просторные апартаменты с отдельной спальней на 13 этаже, вид на море и горы.',
    maxGuests: 4,
    area: 60,
    priceBase: 8500,
    view: 'mixed',
    hasTerrace: true,
    floor: '13',
    beds: '1 диван-кровать + 1 двуспальная',
    features: ['Холодильник', 'Кондиционер', 'Душевая кабина', 'Кухня', 'Балкон'],
    images: ['/images/apartments/diamond-green/1.webp', '/images/apartments/diamond-green/2.webp', '/images/apartments/diamond-green/3.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: false,
    hasWashingMachine: false,
    hasHeatedFloor: false,
    hasSlippers: false,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: false
  },
  {
    id: 'ls-mountain-retreat',
    title: 'LS-MOUNTAIN RETREAT',
    shortDescription: 'Медитация в окружении гор с видом на горы и море',
    description: 'Просторные апартаменты с отдельной спальней на 16 этаже, вид на горы и море.',
    maxGuests: 4,
    area: 60,
    priceBase: 8600,
    view: 'mixed',
    hasTerrace: true,
    floor: '16',
    beds: '1 диван-кровать + 1 двуспальная',
    features: ['Интернет', 'Холодильник', 'Кондиционер', 'Душ', 'Кухня', 'Балкон', 'Полотенца для животных'],
    images: ['/images/apartments/mountain-retreat/1.webp', '/images/apartments/mountain-retreat/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: false,
    hasWashingMachine: false,
    hasHeatedFloor: false,
    hasSlippers: true,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: true
  },
  {
    id: 'ls-wine-and-sunset',
    title: 'LS-WINE AND SUNSET',
    shortDescription: 'Вино и закат с видом на море',
    description: 'Апартаменты с отдельной спальней на 5 этаже, вид на море.',
    maxGuests: 4,
    area: 61,
    priceBase: 8700,
    view: 'sea',
    hasTerrace: true,
    floor: '5',
    beds: '1 диван-кровать + 1 двуспальная',
    features: ['Интернет', 'Холодильник', 'Кондиционер', 'Душевая кабина', 'Пол с подогревом', 'Кухня', 'Балкон', 'Полотенца для животных'],
    images: ['/images/apartments/wine-and-sunset/1.webp', '/images/apartments/wine-and-sunset/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: false,
    hasWashingMachine: false,
    hasHeatedFloor: true,
    hasSlippers: true,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: true
  },
  {
    id: 'ls-lux-white-sands',
    title: 'LS-LUX-WHITE SANDS',
    shortDescription: 'Песочных оттенков с боковым видом на море',
    description: 'Двухуровневые апартаменты с отдельной спальней на 1 этаже, боковой вид на море.',
    maxGuests: 4,
    area: 55,
    priceBase: 8200,
    view: 'sea',
    hasTerrace: true,
    floor: '1',
    beds: '1 диван-кровать + 1 двуспальная',
    features: ['Холодильник', 'Кондиционер', 'Душевая кабина', 'Кухня', 'Балкон', 'Полотенца для животных'],
    images: ['/images/apartments/lux-white-sands/1.webp', '/images/apartments/lux-white-sands/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: false,
    hasWashingMachine: false,
    hasHeatedFloor: false,
    hasSlippers: false,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: true
  },
  {
    id: 'ls-lux-orange',
    title: 'LS-LUX-ORANGE',
    shortDescription: 'В сочных оттенках лета с лучшим видом на море',
    description: 'Апартаменты с отдельной спальней на 14 этаже, лучший вид на море.',
    maxGuests: 5,
    area: 60,
    priceBase: 8900,
    view: 'sea',
    hasTerrace: true,
    floor: '14',
    beds: '1 кресло + 1 диван-кровать + 1 двуспальная',
    features: ['Холодильник', 'Кондиционер', 'Пол с подогревом', 'Кухня', 'Балкон', 'Полотенца для животных'],
    images: ['/images/apartments/lux-orange/1.webp', '/images/apartments/lux-orange/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: false,
    hasWashingMachine: false,
    hasHeatedFloor: true,
    hasSlippers: true,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: true
  },
  {
    id: 'ls-lux-soft-blue',
    title: 'LS-LUX-SOFT BLUE',
    shortDescription: 'Небесных оттенков лета с видом на море и горы',
    description: 'Апартаменты с отдельной спальней на 10 этаже, вид на море и горы.',
    maxGuests: 5,
    area: 60,
    priceBase: 8800,
    view: 'mixed',
    hasTerrace: true,
    floor: '10',
    beds: '1 кресло + 1 диван-кровать + 1 двуспальная',
    features: ['Холодильник', 'Кондиционер', 'Душевая кабина', 'Кухня', 'Балкон'],
    images: ['/images/apartments/lux-soft-blue/1.webp', '/images/apartments/lux-soft-blue/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: false,
    hasWashingMachine: false,
    hasHeatedFloor: false,
    hasSlippers: false,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: false
  },
  {
    id: 'ls-lux-fly-birds',
    title: 'LS-LUX-FLY BIRDS',
    shortDescription: 'Просторные апартаменты',
    description: 'Апартаменты с отдельной спальней.',
    maxGuests: 4,
    area: 60,
    priceBase: 8400,
    view: 'sea',
    hasTerrace: false,
    floor: '',
    beds: '1 диван-кровать + 1 двуспальная',
    features: [],
    images: ['/images/apartments/lux-fly-birds/1.webp', '/images/apartments/lux-fly-birds/2.webp'],
    hasMicrowave: false,
    hasStove: false,
    hasBalcony: false,
    hasCoffeeMachine: false,
    hasWashingMachine: false,
    hasHeatedFloor: false,
    hasSlippers: false,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: false
  },
  {
    id: 'ls-deep-forest',
    title: 'LS-DEEP FOREST',
    shortDescription: 'Крымские ландшафты с видом на горы и город',
    description: 'Апартаменты с отдельной спальней на 3 этаже, вид на горы и город.',
    maxGuests: 5,
    area: 60,
    priceBase: 8500,
    view: 'mixed',
    hasTerrace: true,
    floor: '3',
    beds: '1 кресло + 1 диван-кровать + 1 двуспальная',
    features: ['Интернет', 'Холодильник', 'Кондиционер', 'Душ', 'Кухня', 'Балкон', 'Полотенца для животных'],
    images: ['/images/apartments/deep-forest/1.webp', '/images/apartments/deep-forest/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: false,
    hasWashingMachine: false,
    hasHeatedFloor: false,
    hasSlippers: true,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: true
  },
  {
    id: 'ls-flowers-tea',
    title: 'LS-FLOWERS TEA',
    shortDescription: 'Ароматы цветочного чая с видом на горы и море',
    description: 'Апартаменты с отдельной спальней на 15 этаже, вид на горы и море.',
    maxGuests: 4,
    area: 60,
    priceBase: 8600,
    view: 'mixed',
    hasTerrace: true,
    floor: '15',
    beds: '1 диван-кровать + 1 двуспальная',
    features: ['Интернет', 'Холодильник', 'Кондиционер', 'Душ', 'Кухня', 'Балкон', 'Полотенца для животных'],
    images: ['/images/apartments/flowers-tea/1.webp', '/images/apartments/flowers-tea/2.webp'],
    hasMicrowave: false,
    hasStove: true,
    hasBalcony: true,
    hasCoffeeMachine: false,
    hasWashingMachine: false,
    hasHeatedFloor: false,
    hasSlippers: true,
    hasBathrobes: false,
    hasSafe: false,
    hasWorkplace: false,
    petFriendly: true
  }
];

// Функция для получения апартамента по ID
export function getApartmentById(id: string): Apartment | undefined {
  return APARTMENTS.find(apt => apt.id === id);
}

// Функция для получения всех апартаментов с фильтрацией
export function getApartments(filter?: {
  type?: 'studio' | 'apartment';
  view?: string;
  maxGuests?: number;
  features?: string[];
}): Apartment[] {
  let filtered = [...APARTMENTS];
  
  if (filter?.type) {
    // Примерная фильтрация по типу (можно уточнить)
  }
  
  if (filter?.view) {
    filtered = filtered.filter(apt => apt.view === filter.view);
  }
  
  if (filter?.maxGuests) {
    filtered = filtered.filter(apt => apt.maxGuests >= filter.maxGuests!);
  }
  
  return filtered;
}